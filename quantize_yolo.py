...
class QuantizableYolo(nn.Module):
    def __init__(self, config):
        ...
        self.quant = torch.quantization.QuantStub()
        self.yolo = mbv2_yolo.yolo(config)
        self.dequant = torch.quantization.DeQuantStub()

    def forward(self, x):
        x = self.quant(x)
        x = self.yolo(x)
        x = [self.dequant(out) for out in x]
        return x
...
def main(args):
    ...
    if args.load:
        ...
        if args.model == 'mobilenetv2':
            ...
            assert os.path.isfile(yaml_file), 'Error: no config yaml file found!'
            with open(yaml_file, 'r') as f:
                config = yaml.load(f)
            print(config)
#            model = QuantizableMobileNetV2(model_url)
            model = QuantizableYolo(config=config)
        else:
    ...
    model.eval()
    model.yolo.yolo_losses[0].val_conf = 0.01
    model.yolo.yolo_losses[1].val_conf = 0.01
